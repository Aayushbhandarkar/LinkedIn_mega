...
function Post({ id, author, like, comment, description, image, createdAt }) {
  ...
  useEffect(() => {
    ...
    socket.current.on("likeUpdated", ({ postId, likes }) => {
      if (postId === id) setLikes(likes);
    });
    socket.current.on("commentAdded", ({ postId, comm }) => {
    ...
    return () => {
      socket.current?.off("likeUpdated");
  ...
  // ✅ Remove infinite loop (don’t call getPost() every likes/comments change)
  useEffect(() => {
    getPost();
  }, []);
    ...
    <div className="w-full bg-white rounded-xl shadow-md p-5 flex flex-col gap-4 hover:shadow-lg transition duration-300">
      ...
      <div className="flex justify-around border-t pt-2 text-gray-600 text-[15px] font-medium">
        {!likes.includes(userData?._id) ? (
          ...
          <button
            className="flex items-center gap-2 text-[#0a66c2] font-semibold hover:bg-gray-100 p-2 rounded-lg transition"
            onClick={handleLike}
          >
        ...
        <div className="mt-3">
          ...
          <div className="flex flex-col gap-4 mt-4">
            {comments.map((com) => (
              <div
                key={com._id}
                className="flex gap-3 border-b pb-3 hover:bg-gray-50 transition rounded-lg p-2"
              >
